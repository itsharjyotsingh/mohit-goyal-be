...
COALESCE(SUM(CASE WHEN ep.status = 'paid' THEN ep.amount ELSE 0 END), 0)::int AS total_revenue,
COALESCE(SUM(CASE WHEN ep.status IN ('created','failed','attempted') THEN 1 ELSE 0 END), 0)::int AS attempted_payments,

-- Successful payments: customers with at least one 'paid' row, include payment details (each paid row)
COALESCE((
...
-- Unsuccessful payments: latest non-paid attempt per customer, excluding any customer who has a 'paid' row
COALESCE((
  SELECT JSON_AGG(DISTINCT JSONB_BUILD_OBJECT(
           'customer', JSONB_BUILD_OBJECT(
